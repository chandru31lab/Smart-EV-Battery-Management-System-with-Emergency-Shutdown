...
// ─────────────────────────────────────────────
//  GPIO — MCP2515 CAN Interface
// ─────────────────────────────────────────────
#define CAN_SCK_PIN   21
#define CAN_MISO_PIN  22
#define CAN_MOSI_PIN  23
#define CAN_CS_PIN    15
#define CAN_INT_PIN   16

// Set this to MCP_8MHZ or MCP_16MHZ to match the crystal on your MCP2515 module.
...
unsigned long lastCANTransmit  = 0;
...
const unsigned long CAN_TRANSMIT_MS    = 500;   // transmit CAN telemetry every 500ms

// ═══════════════════════════════════════════════════════════════
//  MCP2515 CAN INITIALIZATION + TELEMETRY
// ═══════════════════════════════════════════════════════════════
...
void setupCAN() {
  ...
  byte result = CAN0.begin(MCP_ANY, CAN_BAUD_RATE, CAN_CLOCK);
  ...
  if (result == CAN_OK) {
    ...
    Serial.printf("[CAN] Initialization FAILED. Error code = %d\n", result);
    Serial.println("[CAN] Check CS pin, SPI wiring, power and crystal setting.");
  }
}

void transmitBatteryCAN() {
  if (!canInitialized) return;

  uint16_t voltage = (uint16_t)constrain((int)(liveVoltage * 100.0f), 0, 65535);
  int16_t current = (int16_t)constrain((int)(liveCurrent * 100.0f), -32768, 32767);

  byte data[8] = {0};
  data[0] = highByte(voltage);
  data[1] = lowByte(voltage);
  data[2] = highByte((uint16_t)current);
  data[3] = lowByte((uint16_t)current);

  byte result = CAN0.sendMsgBuf(0x100, 0, 8, data);
  if (result != CAN_OK) Serial.println("[CAN] Battery frame TX failed");
}

void transmitTemperatureCAN() {
  if (!canInitialized) return;

  int16_t temperature = (int16_t)constrain((int)(liveTemp * 100.0f), -32768, 32767);

  byte data[8] = {0};
  data[0] = highByte((uint16_t)temperature);
  data[1] = lowByte((uint16_t)temperature);
  data[2] = overheating ? 1 : 0;
  data[3] = ds18b20Found ? 1 : 0;

  byte result = CAN0.sendMsgBuf(0x101, 0, 8, data);
  if (result != CAN_OK) Serial.println("[CAN] Temperature frame TX failed");
}

void transmitMotorCAN() {
  if (!canInitialized) return;

  uint16_t rpm = (uint16_t)constrain((int)liveRpm, 0, 65535);
  uint16_t speed = (uint16_t)constrain((int)(liveSpeed * 100.0f), 0, 65535);
  uint16_t frequency = (uint16_t)constrain((int)(liveFrequency * 10.0f), 0, 65535);

  byte data[8] = {0};
  data[0] = highByte(rpm);
  data[1] = lowByte(rpm);
  data[2] = highByte(speed);
  data[3] = lowByte(speed);
  data[4] = highByte(frequency);
  data[5] = lowByte(frequency);
  data[6] = dirFwd ? 1 : 0;

  byte result = CAN0.sendMsgBuf(0x102, 0, 8, data);
  if (result != CAN_OK) Serial.println("[CAN] Motor frame TX failed");
}

void transmitAllCANData() {
  if (!canInitialized) return;

  transmitBatteryCAN();
  transmitTemperatureCAN();
  transmitMotorCAN();

  Serial.printf("[CAN TX] V=%.2fV I=%.2fA T=%.2fC RPM=%.0f SPD=%.2fkm/h F=%.1fHz\n",
                liveVoltage, liveCurrent, liveTemp, liveRpm, liveSpeed, liveFrequency);
}
...
void loop() {
  ...
  // ── Periodic MQTT publish every 500 ms ────────────────────
  if (now - lastPublish >= PUBLISH_INTERVAL) {
    lastPublish = now;
    publishSensorData();
  }

  // ── Periodic CAN telemetry every 500 ms ───────────────────
  if (now - lastCANTransmit >= CAN_TRANSMIT_MS) {
    lastCANTransmit = now;
    transmitAllCANData();
  }
}
